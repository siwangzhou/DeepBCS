...
if mode == 'test':
    rate = 0.4  #the fixed sampling rate {0.01, 0.03, 0.05, 0.1, 0.2, 0.3, 0.4}
    #for Set5   
    ...
    save_paths = 'D:/DeepBCS-master/DataSets/Prediction/%s_FSR/'  %rate
    if os.path.exists(save_paths) == 0:
        os.makedirs(save_paths)
      
    Block_onehot=np.zeros(((1, Block_num_m*Block_num_n, 7)))
    for t in range(Block_onehot.shape[1]):
        if rate==0.01:
            Block_onehot[:, t, :] = [1., 0., 0., 0., 0., 0., 0.] 
        elif rate==0.03:
            Block_onehot[:, t, :] = [0., 1., 0., 0., 0., 0., 0.] 
        elif rate==0.05:
            Block_onehot[:, t, :] = [0., 0., 1., 0., 0., 0., 0.] 
        elif rate==0.1:
            Block_onehot[:, t, :] = [0., 0., 0., 1., 0., 0., 0.] 
        elif rate==0.2:
            Block_onehot[:, t, :] = [0., 0., 0., 0., 1., 0., 0.]
        elif rate==0.3:
            Block_onehot[:, t, :] = [0., 0., 0., 0., 0., 1., 0.]
        elif rate==0.4:
            Block_onehot[:, t, :] = [0., 0., 0., 0., 0., 0., 1.]
...
with tf.Graph().as_default():
    ...
    model_dir = 'D:\DeepBCS-master\Train\Train\Train_model_40layers_WBA'
    ...
    if mode == 'test':
        ...
        duration = np.zeros([nrtest, 1], dtype=np.float32)
